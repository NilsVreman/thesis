...
\begin{tikzpicture}[>=latex]
    ...
    \draw[thick, ->] (v1mc) edge [loop left] node[above, yshift=0.1cm] {$1-\rho$} (v1mc);
    \draw[thick, ->] (v1mc) edge [bend left=45] node[above] {$\rho$} (v2mc);
    \draw[thick, ->] (v2mc) edge [bend left=45] node[below] {$1-\rho$} (v1mc);
    \draw[thick, ->] (v2mc) edge [loop right] node[above, yshift=0.1cm] {$\rho$} (v2mc);
