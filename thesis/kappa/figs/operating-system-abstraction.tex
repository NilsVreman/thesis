...
% CPU
\node[task, minimum width=4.0cm, minimum height=1.8cm] (cpu) at (-0.9+1.5*10*\delta,-2.75) {};
...
% Memory
\node[task, minimum width=2.25cm, minimum height=0.6cm, rotate=90] (mem) at (-1.0+4*10*\delta,-2.5) {Main Memory};

% HW interfaces
\node[task, minimum width=1.5cm, minimum height=0.6cm, rotate=90] (gpio) at (-1.0+4*10*\delta,0.4) {I/O Interface};

% Background 

\begin{scope}[on background layer]
    \node[ultra thick, fill=white, fit=(vt1)(vtn)(cpu)(mem),draw,inner sep=4pt] (vhw) {};
    \draw[thick, dashed] ([yshift=-0.45cm]vhw.west) to ([yshift=-0.45cm,xshift=-1.0cm]vhw.east);
...
\node[circleconn] (ccpu) at ($(cpu)+(0,1.23)$) {};
