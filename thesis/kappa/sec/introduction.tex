...
\question{Add something more to the end here?}{}
...
Similarly, the sensor data is all data collected by the sensors, e.g., the rotational velocity of the wheel or the friction between the wheel and the ground in the taxiing system.
The signals are transmitted via the hardware interfaces on the computational unit responsible for controlling the plant.
...
% Clocks
There exists another discrepancy between the components of the real-time control system that has so far been introduced: the time quantisation.
...
The RTOS clock is then based on the hardware clock; it reads the hardware clock at a frequency specified by the user and wraps it in a virtual layer to make it more application friendly. 
...
The inconsistencies between different clocks is enough of a problem to warrant its own research domain, i.e., clock synchronisation.
...
However, historically the research focus has been to address different types of timing anomalies, such as
%
\begin{itemize}
    \item input (or output) delays,
    \item internal delays,
    \item jitter, or
    \item network delays (or losses).
\end{itemize}
%
\nv{Continue here}
