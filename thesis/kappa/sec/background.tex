...
Every real-time system behave differently to deadline overruns.
...
\end{itemize}
%
\note{MM: Could you double check this paragraph, please?}
...
Prominent for, in particular, embedded systems are the \emph{firm real-time systems}, characterised by being able to overrun a few, but not too many, deadlines before causing critical system failure.
...
What defines a weakly-hard task is that the distribution of deadline hits and misses during a window of $k$ jobs is precisely bounded.
...
With the use of the alphabet and conventional language theoretical notation~\addref{}, a \emph{character} $c_t \in \Sigma$ is defined as the outcome of the $t$-th job.
...
\begin{figure}[t]
    ...
    \caption{\fix{I know this figure sucks, but I wanted an initial placeholder}}
    \label{fig:schedule}
\end{figure}
...
%
\newline\fix{Subscript $t$ is problematic here}\question{MM: Do you have a suggestion for how to solve it?}{}
%
\begin{equation}%
...
Note that the process' stochastic disturbance process $w_t \in \R^{n_w}$ and dynamic matrix $\Wp \in \R^{n_x \times n_w}$ have been added.
The disturbance process $w_t$ is typically a stochastic process with (assumed) known statistical properties, modelling the unknown plant dynamics and exogenous signals.
...
\begin{equation}%
    \label{eq:clsys}%
    \tilde{x}_{t+1} = \clmat\, \tilde{x}_t + \clinp_w\, w_t.
\end{equation}
%
where $\tilde{x}_t$ is the closed-loop system's state vector, $\clmat$ is the closed-loop system matrix responsible for encoding the closed-loop system's dynamics, and
$$\clinp_w = \begin{bmatrix}
    \Wp \\
    0
\end{bmatrix}$$
is the dynamic matrix of the exogenous stochastic variable $w_t$.
...
\begin{equation}%
    \label{eq:tv-clsys}%
    \tilde{x}_{t+1} = \clmat_{\sigma(t)}\, \tilde{x}_t + \clinp_w\, w_t.
\end{equation}
%
...
This thesis facilitates a performance analysis of real-time control systems subject to computational overruns, where the performance is evaluated on the closed-loop system using a stochastic quadratic cost function $J_t$:
%
\begin{equation}%
    \label{eq:cost}%
...
\end{equation}
%
Here $J_t$ is the closed-loop state's cost increment in the $t$-th period and $Q$ is a positive semidefinite weight matrix penalising large closed-loop state deviations.
The model is based on well-known theory for linear stochastic systems~\cite{Astrom:1970, Astrom:1997, Cervin:2019}.

We assume that the desired reference point for the closed-loop state $\tilde{x}_t$ is zero and that the disturbance $w_t$ is an independent, discrete-time, zero mean, Gaussian white noise process with covariance $\E{w_t\,w_t^\T{}} = R_1$.
The assumptions may seem overly restrictive, but we emphasise that more elaborate and advanced disturbance processes (and reference trajectories) can be realised by augmenting the closed-loop system with additional states describing the exogenous dynamics\footnote{In both \note{Paper I and II} we augment the system state to model respectively brown noise (integrated white noise) and reference trajectories.}.
...
Instead, it is easier to first transform the cost $J_t$ into an equivalent form before evaluating it.
We define the closed-loop system's mean value and covariance functions as $m_t$ and $P_t$ respectively
%
\begin{equation}%
    \label{eq:cov}%
    m_t = \E{\tilde{x}_t}, \quad P_t = \cov{\tilde{x}_t} = \E{\tilde{x}_t\, \tilde{x}_t^\T{}} - \E{\tilde{x}_t}\,\E{\tilde{x}_t}^\T{}.
\end{equation}
%
Note that because $\E{w_t} = 0$, the mean value of the state adhere to the following dynamical equation:
%
\begin{equation}%
    m_{t+1} = \E{\clmat\,\tilde{x}_t + \clinp_w\,w_t} = \clmat\, m_t.
\end{equation}
%
From~\eqref{eq:cov} it then follows that~\eqref{eq:cost} can equivalently be written as~\cite{Bates:2011}:
%
\begin{equation}%
    \label{eq:tr-cost}%
    \begin{aligned}
        J_t &= \E{\tilde{x}_t^\T{}\, Q\, \tilde{x}_t} \\
        &= \trace{\E{\tilde{x}_t^\T{}\, Q\, \tilde{x}_t}} \\
        &= \E{\trace{\tilde{x}_t^\T{}\, Q\, \tilde{x}_t}} \\
        &= \E{\trace{Q\, \tilde{x}_t\, \tilde{x}_t^\T{}}} \\
        &= \trace{Q\, \E{\tilde{x}_t\,\tilde{x}_t^\T{}}} \\
        &= \trace{Q\, \left( P_t + m_t\,m_t^\T{} \right)} \\
        &= \trace{Q\, P_t} + m_t^\T{}\, Q\, m_t.
    \end{aligned}
\end{equation}
%
Here, $\trace{\cdot}$ computes the trace of its argument.
If the condition $\E{\tilde{x}_0} = m_0 = 0$ is enforced, the expression can be reduced to solving $J_t = \trace{Q\, P_t}$.

Solving Equation~\eqref{eq:tr-cost} still requires knowledge about how the closed-loop state's covariance $P_t$ propagates in time.
Recalling that $\tilde{x}_t$ and $w_t$ are independent, evolving the covariance results in the following dynamical equation:
%
\begin{equation}%
    \label{eq:cov}%
    \begin{aligned}
        P_{t+1} &= \cov{\tilde{x}_{t+1}} \\
        &= \E{\tilde{x}_{t+1}\,\tilde{x}_{t+1}^\T{}} - \E{\tilde{x}_{t+1}}\,\E{\tilde{x}_{t+1}}^\T{} \\
        &= \E{\tilde{x}_{t+1}\,\tilde{x}_{t+1}^\T{}} - m_{t+1}\,m_{t+1}^\T{} \\
        &= \E{\tilde{x}_{t+1}\,\tilde{x}_{t+1}^\T{}} - \clmat\, m_{t}\,m_{t}^\T{}\,\clmat^\T{} \\
        &= \E{ \left( \clmat\,\tilde{x}_t + \clinp_w\, w_t \right)\left( \clmat\,\tilde{x}_t + \clinp_w\, w_t \right)^\T{} } - \clmat\, m_{t}\,m_{t}^\T{}\,\clmat^\T{} \\
        %&= \E{ \clmat\,\tilde{x}_t\,\tilde{x}_t^\T{}\,\clmat^\T{} + \clmat\,\tilde{x}_t\,w_t^\T{}\,\clinp_w^\T{} + \clinp_w\,w_t\,\tilde{x}_t^\T{}\,\clmat^\T{} + \clinp_w\,w_t\,w_t^\T{}\,\clinp_w^\T{}} - \clmat\, m_{t}\,m_{t}^\T{}\,\clmat^\T{} \\
        &= \clmat\,\E{ \tilde{x}_t\,\tilde{x}_t^\T{} }\,\clmat^\T{} + \clinp_w\,\E{ w_t\,w_t^\T{} }\,\clinp_w^\T{} - \clmat\, m_{t}\,m_{t}^\T{}\,\clmat^\T{} \\
        &= \clmat\,\left( \cov{\tilde{x}_t} + m_t\,m_t^\T{} \right)\,\clmat^\T{} + \clinp_w\,\E{ w_t\,w_t^\T{} }\,\clinp_w^\T{} - \clmat\, m_t\,m_{t}^\T{}\,\clmat^\T{} \\
        &= \clmat\,\cov{\tilde{x}_t}\,\clmat^\T{} + \clinp_w\,\E{ w_t\,w_t^\T{} }\,\clinp_w^\T{} \\
        &= \clmat\,P_t\,\clmat^\T{} + \clinp_w\,R_1\,\clinp_w^\T{}
    \end{aligned}
\end{equation}
%
...
When the control task starts experiencing deadline overruns, the switching dynamics alters the performance analysis accordingly, i.e., swapping $\clmat$ for $\clmat_{\sigma\funof{t}}$ in Equation~\eqref{eq:cov}\footnote{In \note{Papers I and II} we make additional alterations to the performance analysis to properly facilitate it under the specific system conditions.}.
...
For instance, \code{JitterBug}~\cite{Lincoln:2002} is a tool to evaluate precisely the quadratic cost function considered in this thesis~\eqref{eq:tr-cost} for mixed continuous-/discrete-time system components.
The \code{JitterBug} toolbox models the execution time of different components as stochastic variables (in a Markov process) with known probabilistic properties.
...
As a response, the \code{JitterTime} toolbox for Matlab\footnote{The \code{JitterTime} toolbox has also been translated to the open-source programming language Julia~\cite{Julia:2017} by personnel affiliated with the department of automatic control, Lund University, e.g., this thesis' author: \url{https://github.com/X-N-C/JitterTime.jl}.} was developed to provide similar functionality as \code{JitterBug}, but with more freedom for the user to design the different components' timing models.
