...
Since the soft real-time systems typically do not crash with a finite number of deadline overruns, it is common that they are modelled using stochastic processes.
For instance, the simplest models assume that jobs' deadline outcomes are independent and identically distributed random variables, i.e., that the outcome of each job only depend on the probability that this specific job overrun its deadline.
Intuitively, this assumption is too simplistic for real systems and instead stochastic models for the task execution were developed based on, e.g., Markov chains~\cite{Liu:2005, Friebe:2022, Abeni:2017} and task chains~\cite{Manolache:2004, Liu:2010}.
...
Additionally, $I_{n}$ is the size $n \times n $ identity matrix and $\otimes$ is the Kronecker product.
Then, the following theorem holds~\cite{Costa:2005}:
%
...
Finding the spectral radius of a matrix is intuitively much faster than, e.g., the joint spectral radius (which has already been discussed to be undecidable).
