...
\end{equation}
%
Here, the matrices $\Ap_c(t) \in \R^{n_x \times n_x}$ and $\Bp_c(t) \in \R^{n_x \times n_u}$ govern the state evolution of the plant while $\Cp_c(t) \in \R^{n_y \times n_x}$ and $\Dp_c(t) \in \R^{n_y \times n_u}$ describe its output process.
...
This does in turn also change the closed-loop dynamics from Equation~\eqref{eq:clsys} to:
%
\begin{equation}%
    \label{eq:tv-clsys}%
