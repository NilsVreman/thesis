...
Although this thesis does not discern different hardware architectures from one another, it is appropriate to talk about \emph{microcontrollers} and \emph{embedded systems}, in particular due to their prevalence in real-time control systems. 
...
% RTOS, threads
To simplify the interface with the hardware while guaranteeing timeliness, a real-time operating system is commonly employed.
...
\begin{itemize}
    ...
    \item \tS{} -- Under the \tS{} strategy (sometimes referred to as \code{Skip-Next} or \code{Continue}), the job overrunning its deadline is allowed to continue executing until completion.
    ...
    \item \tK{} -- If a job overruns its deadline under the \tK{} strategy (sometimes referred to as \code{Abort}), the job is immediately terminated allowing the subsequent job to be released and activated on time.
...
Similarly to how the plant is described using a dynamical system on state-space form, controllers are typically defined on the same format (denoted the controller's \emph{control law}).
...
\begin{theorem}[Mean-Square Stability]%
    ...
    %
    \begin{equation*}%
        \Psi = \left( \Pi^\T{} \otimes I_{n^2} \right) \cdot \blkdiag{ \left\{ \clmat_i^\T{} \otimes \clmat_i \right\}_{i \in \{1, 2, \ldots, N \}} }.
    \end{equation*}
\end{theorem}
%
...
This is one of the main advantages with the stochastic certificate received by the MSS in comparison to the deterministic one received by computing the JSR.
...
The model is based on well-known theory for linear stochastic systems~\cite{Astrom:1970, Astrom:1997, Nilsson:1998b, Cervin:2019}.
...
For instance, \code{JitterBug}~\cite{Lincoln:2002} and \code{JitterTime}~\cite{Cervin:2019} are tools to precisely evaluate the quadratic cost function considered in this thesis~\eqref{eq:tr-cost} for mixed continuous-/discrete-time system components.%
%\footnote{The \code{JitterBug} toolbox models the execution time of different components as stochastic variables (in a Markov process) with known probabilistic properties.
