...
\end{equation}
%
Here, the \emph{control state vector} $z_t \in \R^{n_z}$ represent the controller's dynamics, while the functions $p$ and $q$ govern the dynamical behaviour of the controller.
Similar to the plant, if $p$ and $q$ can be expressed as linear combinations of their arguments, the controller $\ctrler$ is said to be linear.
%
\begin{equation}%
...
There exists a vast treatise on the synthesis and analysis of both linear and non-linear control laws, the breadth of which is too large to review in any thesis.
\nv{give a few known examples}
