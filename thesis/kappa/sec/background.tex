...
To orchestrate the tasks and to provide them with the correct resources, a scheduler is employed.
...
Included in the scheduler is a \emph{scheduling algorithm}, responsible for deciding how to allocate the resources.
Classical examples of scheduling algorithms are
\begin{enumerate*}[label=(\roman*)]
    \item fixed priority preemptive (FPP); \nv{Some description}
    \item earliest deadline first (EDF); and \nv{Some description}
    \item round-robin (RR). \nv{Some description}
\end{enumerate*}
Depending on the choice of algorithm, the real-time system's execution pattern may vastly differ.
For single-core processors, \nv{mention something about FPP, EDF, and RR and what they do}
...
{Should I:
\begin{itemize}
    \item add something about release jitter and latency?
...
\nv{CONTINUE WITH: why do deadline misses occur?}
