...
The system is said to be linear time-invariant (LTI) if none of the matrices in Equation~\eqref{eq:linear-state-space} are time-dependent.
...
The DAC is located near the actuators and translates the discrete value received from the controller to the actuator commands.
Similarly, the ADC transforms the sensor values to discrete signals to be sent to the controlling hardware.
...
The sensors are then frequently configured to sample the plant at the discrete time instants when the control signal to the plant changes.
...
\newline\fix{$A$ same in discrete and continuous case.}
...
\end{equation}
%
Here, the variable subscripts counts the discrete time samples since system startup, i.e., $x_t = x(t\cdot \Ts)$ and $x_{t+1} = x(t\cdot \Ts + \Ts)$.
\fix{Add noise and disturbances here.}

Similarly to how the plant can be described by a dynamical system on state-space form, controllers are also typically defined in the format of Equation~\eqref{eq:state-space} (denoted the controller's \emph{control law}).
However, Section~\ref{sec:background:rts} described how the control algorithm is implemented as a function governed by a task executing in the RTOS; thus, modern controllers are unlikely to be continuous.
The discrete, time-invariant state-space representation of a general control law is then:
%
\begin{equation}%
...
Similar to the plant, if $p$ and $q$ can be expressed as linear combinations of their arguments, i.e., $p(z_t,\, y_t) = F\,z_t+G\,y_t$ and $q(z_t,\, y_t) = H\,z_t+K\,y_t$, then the controller $\ctrler$ is said to be linear.
%
\begin{equation}%
...
Where LQR optimises the control over a full window, MPC provides a receding time horizon solution while also supporting constraints on the trajectory, control state, and control signal.
\question{Add more?}{}

Control theory is dedicated to developing models and algorithms for designing $p$ and $q$ such that the control system's state follows a desired trajectory while minimising undesirable effects such as noise, disturbances, and large actuator commands.
...
The sensor signals that are received at the controller are processed by the control algorithm before the control signal is transmitted to the actuators; routing signals back into the plant like this is called \emph{feedback control} (or \emph{closed-loop control}).
\question{Figure here to describe this?}{}
...
\end{equation}
%
where $\tilde{x}_t$ is the closed-loop system's state vector and $\clmat$ is the closed-loop system's matrix, responsible for encoding the closed-loop system's dynamics.

\subsubsection*{Controllers as Tasks}%
%
...
\nv{Continue here by explaining:
\begin{itemize}
    \item Explain how a control law is implemented as a task
    \item What happens when the control task overruns its deadline.
\end{itemize}}
