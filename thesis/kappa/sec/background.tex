...
Intuitively, this assumption is too simplistic for real systems and instead stochastic models for the task execution were developed based on, for instance, Markov chains~\cite{Liu:2005, Friebe:2022, Abeni:2017, Lincoln:2003} and task chains~\cite{Manolache:2004, Liu:2010}, where the transitions between states are probabilistic.
...
% Mention some types of performance analysis
...
%
This thesis facilitates a performance analysis of real-time control systems subject to computational overruns, where the performance is evaluated using a \emph{stochastic cost function on quadratic form} $J_t$:
%
\begin{equation}%
    \label{eq:cost}%
...
\end{equation}
%
The performance analysis is summarised in the following definition:
%
\begin{definition}[Stochastic Quadratic Cost Function]%
    \label{def:cost}%
    \nv{TODO}
\end{definition}
...
%Jittertime, Jitterbug, Truetime

% Go into JitterTime's analysis


\nv{Regardless of the performance analysis chosen, if they penalise the same things they should show the same behaviour}
\nv{%
