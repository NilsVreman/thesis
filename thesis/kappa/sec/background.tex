...
Since the RTOS suspend the execution of the active task whenever an interrupt is triggered (even in the middle of a time slice), it is crucial that the ISR is quick to execute to avoid stalling the processor.
However, if the callback function takes too long to execute or if the ISR is triggered too often, it can cause significant time delays in the schedule execution.
