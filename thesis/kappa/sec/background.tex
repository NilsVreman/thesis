...
\question{Is this too big of an oversimplification? Should I add something else?}{}
...
\begin{table}[h]
    ...
    \begin{tabular}{c|c}
        Hurwitz & Schur \\ \hline
        $\underset{i}{\max}{\funof{\mathrm{Re}\funof{\eig{}{\Ap_c}}}} < 0$ & $\underset{i}{\max}{\abs{\eig{}{\Ap}}} < 1$
    \end{tabular}
\end{table}
%
\newline\noindent
where $\eig{}{\cdot}$ computes a vector of eigenvalues and $\mathrm{Re}\funof{\cdot}$ extracts the real-part of a complex number (element-wise when applied to a vector).
...
Essentially, the stability criterion is based on deriving an energy function $V$ (denoted the \emph{Lyapunov function}) for the system under consideration.
The following theorem is a paraphrased version of the discrete-time Lyapunov stability criterion~\cite{Astrom:1997}~\addref{}.
%
\begin{theorem}[Lyapunov's Second Method for Stability]
    \label{thm:lyapunov}%
    Let $V(x)$ be a Lyapunov function candidate for the system $x_{t+1} = f(x_t)$ which has a point of equilibrium in $x_t = x^e$, i.e., $V(x)$ is a continuous function in $x$ satisfying the following:
    \begin{itemize}
        \item $V(x) = 0$ if and only if $x = x^e$,
        \item $V(x) > 0$ if and only if $x \neq x^e$,
        \item $V(x) \rightarrow \infty$ if $\norm{x} \rightarrow \infty$,
        \item $V(f(x)) - V(x) \leq 0,\; \forall x$.
    \end{itemize}
    %
    Then the following are equivalent:
    %
    \begin{enumerate}
        \item The point of equilibrium $x^e$ is globally asymptotically stable (GAS).
        \item The Lyapunov function candidate $V(x)$ exists.
    \end{enumerate}
\end{theorem}
%
A visual argument involve thinking of a physical system, for instance, an oscillating pendulum; it will always move toward the position with the lowest energy, finally reaching its resting state when it is hanging freely downwards, i.e., where the potential energy of the pendulum is minimal.

Lyapunov stability is a powerful tool in control, mainly because it does not require as strong assumptions on the system $f(x)$ as, for instance, the Schur stability criterion.
However, the main threshold to overcome before applying Theorem~\ref{thm:lyapunov} is finding a suitable Lyapunov candidate function $V(x)$, which in general is a complex problem.
