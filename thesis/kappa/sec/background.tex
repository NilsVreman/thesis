...
\begin{equation}%
    ...
    \begin{aligned}
        ...
        \textrm{s.t.}\quad&
        \left\{\begin{matrix*}[l]
                p\funof{x} &\in \mathrm{SOS}_{2d},\\
                \gamma^{2d} p\funof{x} - p\funof{\clmat_i x} &\in \mathrm{SOS}_{2d},\; \clmat_i \in \Aa.
        \end{matrix*}\right.
    \end{aligned}
\end{equation}
%
...
Aside from the obvious stability necessity, such requirements typically involve (but are not limited to):
%
\begin{enumerate*}[label=(\roman*)]
...
\end{enumerate*}
%
Ideally, any controller's ultimate goal is to track the desired system trajectory perfectly and immediately whilst rejecting all disturbances and keeping the control effort to a minimum.
...
The trade-off expresses itself as a limit on how efficiently the desired setpoint can be tracked if disturbances and unmodeled dynamics are also to be effectively rejected.
Conclusively, how the control objectives are valued is dependent on the particular application.
...
The analysis is typically simulation-based, meaning that the performance is evaluated on a virtual replica of the system.
Intuitively, performance analysis methods provide a rough estimate of how well the controller will perform when connected to the actual system.
To get more accurate results, it is necessary to run the controller also on the physical hardware.
