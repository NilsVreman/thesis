...
Common for both the automata and MC is that we denote the underlying graph $\GG{}$, defined by the set of \emph{vertices} (or \emph{states}) $\VV{}$ and the set of \emph{edges} (or \emph{transitions}) $\EE{}$.
However, the vertices and transitions symbolise different things for the two cases.

The automaton model is used to represent the feasible sequences of deadline hits and misses for the weakly-hard constraints\footnote{Related to \note{Papers III and IV}.}.
...
In the stochastic modelling of computational overruns\footnote{Related to \note{Papers II and V}.}, the Markov chain model will instead be adopted.
\nv{CONTINUE HERE}




% Examples of automata for monitoring and markov chains
