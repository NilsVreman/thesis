...
\begin{definition}[Joint Spectral Radius]%
    \label{def:jsr}%
    Given a bounded \mm{What does ``bounded'' mean here?} set of matrices $\Aa$, the \emph{joint spectral radius} (JSR) of $\Aa$ is defined as:
    \begin{equation*}
...
\end{theorem}
%
Furthermore, Theorem~\ref{thm:switching-stability} raises an interesting question about the converse case: which is the switching sequence $\clmat \in \Aa^k$ \mm{I think I am a bit confused by $\clmat \in \Aa^k$ -- I guess what confuses me is the use of $\clmat$ when you also have $\clmat_1$ in the previous equation, and you also wrote $\clmat_{WC}$ above. Here $\clmat$ is one of the matrices that belong to $\Aa^k$ but then I would somehow expect it to have a subscript, and the subscript cannot be $i$ or anything like it (I get it) because then it would be the $i$-th of the $k$ matrices. Maybe you could reuse the WC subscript? Although it is a slight abuse in notation because it is not necessarily the only one that exposes instability. Maybe you could use $\bar{\clmat}$?} resulting in $\rho\funof{\Aa} \geq 1$?
...
\mm{I have the feeling that you need a header here to separate deterministic from probabilistic. Which also means that you needed a header before for deterministic and you need a sentence introducing the fact that most of the thesis is set in the deterministic framework, but you have also investigated the probabilistic one.}
...
When the control task starts experiencing deadline overruns, the switching dynamics alters the performance analysis accordingly, i.e., swapping $\clmat$ for $\clmat_{\theta_k}$ in Equation~\eqref{eq:cov}.\footnote{In \note{Papers I and II} we make additional alterations to the performance analysis to properly facilitate it under the specific system conditions.}
...
\mm{I think the \code{JitterBug} and \code{JitterTime} part comes a bit out of the blue, and provides a level of details that I am not sure is consistent with the rest. It is also not clear if this is your work or simply something that the community did and you are using in your papers. Also not clear if you are using it in your papers or not at this point.}

\mm{You should have a description of \tZ{} and \tH{} near the \tK{}, \tS{} and \tQ{}.}
