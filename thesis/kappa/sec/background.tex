...
Thus, cache evictions are limited to the specific cache memory regions that are shared among multiple tasks.
...
Typically, each peripheral is assigned to an \emph{I/O port} in the device, i.e., a unique address to communicate through. 
The port is then used by both the peripheral and the tasks to identify where to transmit data to and read data from.

Separately from the I/O port, the \emph{communication protocol} defines the rules used to decipher and encrypt the packets sent between the peripherals and the hardware over the \emph{communication channel}.
...
Information transmitted over a network (wired or wireless) is generally represented as a set of bits (ones and zeroes) to be read in series or parallel; without loss of generality as well as for simplicity, we will only mention the serial case.
...
Since the binary representation of \code{R} is $01010010$, the encoded characters packet representation (to be sent over the network) is then:
%
\begin{equation*}
    0 \;\, 01010010 \;\, 1.
\end{equation*}
