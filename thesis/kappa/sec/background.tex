...
On the contrary, the \tZ{} actuation mode stops acting on the plant if no new control command is received.\footnote{We show in \note{Paper~I} that the choice of actuation mode can significantly affect both stability and performance properties of the physical system, when the controller is subject to computational overruns.}
...
\end{theorem}
%
Furthermore, Theorem~\ref{thm:switching-stability} raises an interesting question about the converse case: which is the switching sequence $\clmat_1 \cdots \clmat_k \in \Aa^k$ resulting in $\rho\funof{\Aa} \geq 1$? %\mm{I think I am a bit confused by $\clmat \in \Aa^k$ -- I guess what confuses me is the use of $\clmat$ when you also have $\clmat_1$ in the previous equation, and you also wrote $\clmat_{WC}$ above. Here $\clmat$ is one of the matrices that belong to $\Aa^k$ but then I would somehow expect it to have a subscript, and the subscript cannot be $i$ or anything like it (I get it) because then it would be the $i$-th of the $k$ matrices. Maybe you could reuse the WC subscript? Although it is a slight abuse in notation because it is not necessarily the only one that exposes instability. Maybe you could use $\bar{\clmat}$?} 
