...
\end{equation}
%
Here $x_k\in\R^{n_x}$, $u_k\in\R^{n_u}$, and $y_k\in\R^{n_y}$ are the plant state, the control signal and the plant output, sampled at time $k\cdot \Ts$, $\Ts$ is the sampling period, and $k\in\N$.
...
%
Given $N\in\N\setminus \left\{ 0 \right\} $ and a set of matrices $\Aa = \{\clmat{}_1, \ldots, \clmat{}_N \} \subseteq \R^{n\times n}$, under the hypothesis of arbitrary switching over any sequence $w=\seq{c_1,c_2,\dots}$ of indices of matrices in $\Aa$, the JSR of $\Aa$ is defined by:
\begin{equation}
    \label{jsr}
