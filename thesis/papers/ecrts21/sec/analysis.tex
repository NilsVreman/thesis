...
%%%%%%%%%%%%%%% Old Version %%%%%%%%%%%%%%%
%From an industrial viewpoint, the proposed fault model is highly
%relevant. The common approach is to treat the faults as
%stochastic~\cite{reliabook,montgomery2009introduction, Teich:2017}\anton{Process this paragraph to say "our model stands out from the rest, due to how we analyze consecutive misses in ONE burst"}. 
%However, during the operation of a control system, faults can be 
%caused by events like network connection problems (cutting the 
%connection between the sensor and the controller). This is not 
%a stochastic event\anton{Again I'm not sure what you mean here}, and in particular deadlines are not missed 
%independently. The controller does not execute correctly until 
%the connection is restored. The analysis methods we propose allows 
%us to address realistic situations like connection problems, 
%and provide tighter and more realistic bounds on the stability 
%and performance of control systems in the presence of faults.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{definition}[\titlecap{Weakly Hard fault model with burst of misses}]
...
% The analysis is exact for \textcolor{red}{cyclic patterns} of misses and hits. 
% We compare this exact analysis with the state of the art,
% discussing similarities and differences. This analysis holds for
% a given number of misses $\nummisses$ followed by a recovery
% interval of hits. However, we would like it to hold for any number of
% consecutive misses up to $\nummisses$ followed by a recovery interval. 
% In order to address this additional requirement, we conclude
% the section by proposing a stability analysis that combines the exact
% analysis with the principle behind the switching-system analysis
% presented in~\cite{Maggio:2020}.
...
%We can hence use the criterion in order to find the smallest number of deadline hits $\numhits$ such that $\nummisses$ consecutive misses do not destabilise the system. 
...
%It is important to note that $\numhits$ is the number of consecutive hits necessary for the system to recover to such an extent that another burst interval would not destabilise the system. 
...
% This stability analysis is fairly simple in nature, and applies
% precisely to the model presented in Definition~\ref{def:task-model}.
\begin{definition}[\titlecap{\nilsstability{} stability analysis}]
...
\begin{definition}[\titlecap{\switchingstability{} stability analysis}]
...
\begin{definition}[\titlecap{Performance recovery interval}]
\label{def:recovery-lenght-interval}
%
We define the recovery length interval
$\recoverylengthinterval_{\strat}$ as the smallest $\numhits$
such that~\eqref{eq:cost-threshold} is satisfied for all $k \geq
\numhits$ when using $\strat$ to handle deadline
misses.
%
\end{definition}

\begin{definition}[\titlecap{Maximum normalised cost}]\label{def:maximum-system-cost}
%
...
\end{equation}
%
where $J_{k,\strat}$ is the cost computed according
to~\eqref{eq:covartocost} when using $\strat$ to handle the
deadline misses.
%
\end{definition}
...
\node[rectangle, draw, fill=white, align=center] at (axis cs:15,4.5) {$\strat = KZ$\\\tikz{\fill[white] (0,0) rectangle (1,0.75);\draw[blue, ultra thick] (0,0.25) -- (1,0.25); \fill[blue] (0.5,0.25) circle (0.175);}$\,\,J_{k,\strat}/J_\infty=J_{k,KZ}/J_\infty$};
\draw[black] (0,0) rectangle (20,6.25);
\node[clip=false, anchor=west] at (axis cs:20.55,1) {$2\,\varepsilon$ band};
\addplot table [col sep=comma, x=T, y=J] {data/definitions.csv};
\draw[dashed, black] (axis cs:0,5.4692) -- (axis cs:5,5.4692) node[right, xshift=1mm, blue] {$J_{M,\strat} = J_{M,KZ}$};

\draw[<->, blue, thick] (5,0) -- (5,5.4692);
\draw[<->, red, thick] (0,3) -- node[above] {$\nummisses$} (3,3);
\draw[<->, green!80!black, thick] (3,3) -- node[above] {$\recoverylengthinterval_{\strat} = \recoverylengthinterval_{KZ}$} (13,3);
...
Compared to the stability analysis, the performance analysis also
