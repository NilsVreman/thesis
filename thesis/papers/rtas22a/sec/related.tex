...
%To distinguish the contribution of this paper from the existing treatise, we offer a discussion on the scientific literature. 
...
%We also offer a stochastic performance analysis based on a probabilistic characterisation of the deadline miss patterns. 
%\revise{The only previous work using a similar analysis approach is~\cite{Pazzaglia:2019}, where it was instead used for robust controller design.} \ac{Not true, such analysis goes back at least to Nilsson 1998 \ldots}
...
\begin{enumerate}[label=(\roman*)]
    ...
    % \item Conservative results---i.e., the system performs worse under nominal operating conditions~\cite{Schinkel:2006, Chakraborty:2012}.
\end{enumerate}
...
\begin{example}[Motivating Example]
    ...
    %As can be seen from the following equation, this controller includes a state $z$, making it dynamic:
    \begin{equation*}
...
%This type of analysis is complex to construct and compute, \revise{and results rely on the use of Lyapunov theory~\cite{Khalil:2015}}.
%While being \revise{very generally applicable}, the theory only provides \emph{sufficient} conditions for stability.
%As a consequence, there could be cases that are stable but, since the theory does not reach a certain verdict, are not considered as such.
%This results in a conservative analysis.
%An alternative is the use of the joint spectral radius~\cite{Maggio:2020, Pazzaglia:2021} (note that the latter is the only previous work that considers dynamic controllers).
%However, the joint spectral radius cannot be exactly computed and approximations are needed, resulting again in conservative results.
%
%In general, stability certificates for dynamic controllers are a complex topic.
...
%\revise{ assuming an accurate model of the plant}. \revise{This analysis is inherently conservative, as it only provides sufficient conditions for stability under \emph{worst-case} conditions.}
